...
DROP INDEX I_compuesto ON libros;
-- No se puede borrar porque afecta la foreign key
...
CREATE UNIQUE INDEX id_libros_id_editorial on libros (id_editorial);

-- Ejercicio 5: 
-- ¿Se puede usar alter para resolver el ejercicio anterior?
-- No se puede usar un alter porque no se puede borrar un indice compuesto por el foreign key 

-- Ejercicio 6:
-- Crea un índice único en la columna id_editorial de la tabla editoriales.
CREATE UNIQUE INDEX I_unico
...
-- porque ya tengo una clave primaria predeterminada en la creacion de la tabla
